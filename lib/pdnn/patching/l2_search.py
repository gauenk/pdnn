...
# -- sim search import --
from vnlb.gpu.sim_search import compute_l2norm_cuda

def exec_l2_search(srch_img,srch_inds,k,sigma,ps,pt):
    ...
    for b in range(B):
        inds_b = exec_l2_search_burst(srch_img[b],srch_inds[b],k,sigma,ps,pt)
        inds[b,...] = inds_b

    return inds

def exec_l2_search_burst(srch_img,srch_inds,k,sigma,ps,pt):

    # -- fixed params --
    device = srch_img.device
    T,C,H,W = srch_img.shape
    fflow = th.zeros(T,C,H,W,dtype=th.float,device=device)
    bflow = fflow
    step_s = 1
    w_s = 27
    nWt_f = 6
